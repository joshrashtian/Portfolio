    ...
    <motion.div className="py-3 pb-10 mx-14 my-10 h-screen">
      {posts.map((post, index) => (
        <motion.div
          ...
          className="mx-20 rounded-2xl bg-zinc-200 my-5 p-4 py-8 pb-14"
        >
          <h1 className={`${post.headingcss} text-5xl m-4 ml-2 font-eudoxus`}>
            {post.title}
          </h1>
          
          <div className="flex items-center my-2 justify-evenly">
            <div className="flex">
            <h1 className=" font-eudoxus mx-2 my-2 rounded-lg">
            {post.postedAt}
            </h1>
            </div>
            ...
            {post.tags != undefined
              ? post.tags.map((tag, index) => (
                  <div
                    key={index}
                    className="  bg-zinc-300 p-1 px-2 rounded-xl"
                  >
